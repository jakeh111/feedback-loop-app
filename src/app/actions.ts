...
export async function deleteAllUserData(userId: string) {
  if (!userId) {
    throw new Error('User ID is required to delete data.');
  }
  ...
  try {
    ...
    // 1. Delete all sub-collections (comments) first
    for (const trackDoc of tracksSnapshot.docs) {
      const commentsRef = trackDoc.ref.collection('comments');
    ...
    // 2. Delete all track documents
    const tracksBatch = firestore.batch();
    tracksSnapshot.forEach(doc => {
      tracksBatch.delete(doc.ref);
    });
    await tracksBatch.commit();
    
    // 3. Delete all files from storage
    const bucket = storage.bucket();
    const [files] = await bucket.getFiles({ prefix: `tracks/${userId}/` });
    await Promise.all(files.map(file => file.delete()));
    
  } catch(error) {
    console.error(`Error deleting all data for user ${userId}:`, error);
    if (error instanceof Error) {
      throw new Error(error.message);
    }
    throw error;
